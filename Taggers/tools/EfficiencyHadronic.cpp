/*
g++ -Wall -o EfficiencyHadronic `root-config --cflags --glibs` -L $ROOTSYS/lib -lRooFitCore -lFoam -lMinuit -lMathMore CMS_lumi.C tdrstyle.C EfficiencyHadronic.cpp
*/
...
void MakeAllPlots(const int n, TH1F** histo, TString title, bool Normalize=0)
{
	...
	int Color[n] = {kOrange+3, kOrange-7, kRed+2, kRed, kOrange, kYellow, kSpring, kGreen+1, kGreen+3, kTeal, kAzure+5, kAzure, kViolet+2, kRed+1, kAzure, kViolet+2, kRed+1};
	...
	float integratedEfficiency1 = histo[14]->GetSumOfWeights()/histo[0]->GetSumOfWeights();
	stringstream ss1;
	...
	allMVA -> SaveAs(title + "MVA.pdf");
	allMVA -> SaveAs(title + "MVA.png");





	TCanvas* allCutBased = new TCanvas();
	allCutBased -> cd();
	...
	for(int i=0; i<10; i++)
		leg2 -> AddEntry(histo[i], histo[i]->GetTitle(), "l");
	for(int i=14; i<n; i++)
		leg2 -> AddEntry(histo[i], histo[i]->GetTitle(), "l");

	leg2 -> Draw("SAME");

	float integratedEfficiency2 = histo[16]->GetSumOfWeights()/histo[0]->GetSumOfWeights();
	stringstream ss2;
	ss2 << "#varepsilon: " << std::setprecision(4) << integratedEfficiency2*100 << "%" << endl;
	TString string2 = ss2.str();

	TLatex* tex2 = new TLatex(0.65, 0.50, string2);
	tex2 -> SetNDC();
	tex2 -> Draw("SAME");


	CMS_lumi(allCutBased, 0, 0);

	allCutBased -> SaveAs(title + "CutBased.pdf");
	allCutBased -> SaveAs(title + "CutBased.png");
	...
	RecoMVA -> SaveAs(title + "RecoMVA.pdf");
	RecoMVA -> SaveAs(title + "RecoMVA.png");
	...
	TCanvas* RecoCutBased = new TCanvas();
	RecoCutBased -> cd();
	...
	TLegend* leg4 = new TLegend(0.6, 0.65, 0.9, 0.9);
	for(int i=3; i<11; i++)
		leg4 -> AddEntry(histo[i], histo[i]->GetTitle(), "l");
	for(int i=15; i<n; i++)
		leg4 -> AddEntry(histo[i], histo[i]->GetTitle(), "l");

	leg4 -> Draw("SAME");

	CMS_lumi(RecoCutBased, 0, 0);

	RecoCutBased -> SaveAs(title + "RecoCutBased.pdf");
	RecoCutBased -> SaveAs(title + "RecoCutBased.png");
...
int main(int argc, char *argv[])
{
	...
	TString SignalFold = "/afs/cern.ch/work/a/abeschi/ttHEfficiency_v4/";

	TChain* ttH;
	
	float lumiFactor = 35.9;

	ttH = new TChain("TTHHadronicEfficiencyDumper/trees/tth_13TeV_all");
	...
	float jet_hadronFlavour[9] = {0., 0., 0., 0., 0., 0., 0., 0., 0.};
	...
	ttH -> SetBranchAddress("hasRecoLepton", &hasRecoLepton);
	...
	{	ttH -> SetBranchAddress(("jet_pt"+ std::to_string(i)).c_str(), &jet_pt[i-1]);
		...
		ttH -> SetBranchAddress(("jet_hadronFlavour"+ std::to_string(i)).c_str(), &jet_hadronFlavour[i-1]);
...
*/
	...
	{	
		...
		for(int i=0; i<9; i++)
		{
			if(jet_pt[i]<jetPtThreshold || abs(jet_eta[i])>jetEtaThreshold) continue;
			njets++;
		...
		if(dipho_leadPt>0 &&passHLT==1 && passPreselection==1 && dipho_leadPt>0.5*dipho_mass && dipho_subleadPt>0.25*dipho_mass && hasRecoLepton==0)
		...
		if(dipho_leadPt>0 &&passHLT==1 && passPreselection==1 && dipho_leadPt>0.5*dipho_mass && dipho_subleadPt>0.25*dipho_mass && hasRecoLepton==0 && njets>=3)
		...
		if(dipho_leadPt>0 &&passHLT==1 && passPreselection==1 && dipho_leadPt>0.5*dipho_mass && dipho_subleadPt>0.25*dipho_mass && hasRecoLepton==0 && njets>=3 && nbjetsLoose>=1)
		...
		if(dipho_leadPt>0 &&passHLT==1 && passPreselection==1 && dipho_leadPt>0.5*dipho_mass && dipho_subleadPt>0.25*dipho_mass && hasRecoLepton==0 && njets>=3 && nbjetsLoose>=1 && ttHMVA>0.75)
		...
		if(dipho_leadPt>0 &&passHLT==1 && passPreselection==1 && dipho_leadPt>0.5*dipho_mass && dipho_subleadPt>0.25*dipho_mass && hasRecoLepton==0 && njets>=3 && nbjetsLoose>=1 && ttHMVA>0.75 && dipho_mva>0.4)
		{	mass[13] -> Fill(dipho_mass, weight*lumiFactor);
			eta[13] -> Fill((*Reco1+*Reco2).Eta(), weight*lumiFactor);
			pt[13] -> Fill(dipho_sumpt, weight*lumiFactor);
		}

		if(dipho_leadPt>0 &&passHLT==1 && passPreselection==1 && dipho_leadPt>0.5*dipho_mass && dipho_subleadPt>0.25*dipho_mass && hasRecoLepton==0 && njets>=5)
		{	mass[14] -> Fill(dipho_mass, weight*lumiFactor);
			eta[14] -> Fill((*Reco1+*Reco2).Eta(), weight*lumiFactor);
			pt[14] -> Fill(dipho_sumpt, weight*lumiFactor);
		}

		if(dipho_leadPt>0 &&passHLT==1 && passPreselection==1 && dipho_leadPt>0.5*dipho_mass && dipho_subleadPt>0.25*dipho_mass && hasRecoLepton==0 && njets>=5 && nbjetsMedium>=1)
		{	mass[15] -> Fill(dipho_mass, weight*lumiFactor);
			eta[15] -> Fill((*Reco1+*Reco2).Eta(), weight*lumiFactor);
			pt[15] -> Fill(dipho_sumpt, weight*lumiFactor);
		}

		if(dipho_leadPt>0 &&passHLT==1 && passPreselection==1 && dipho_leadPt>0.5*dipho_mass && dipho_subleadPt>0.25*dipho_mass && hasRecoLepton==0 && njets>=5 && nbjetsMedium>=1 && dipho_mva>0.5)
		{	mass[16] -> Fill(dipho_mass, weight*lumiFactor);
			eta[16] -> Fill((*Reco1+*Reco2).Eta(), weight*lumiFactor);
			pt[16] -> Fill(dipho_sumpt, weight*lumiFactor);
		}
	...
	string name[n] = { "Genrated events                          : ", "Not in EB-EE gap                         : ", "Geometric acceptance                     : ", "Acceptance and reconstruction efficiency : ", "HLT efficiency                           : ", "Preselection                             : ", "Photons pT > 33(25) GeV                  : ", "Photons pT > 0.33(0.25) diphoton mass    : ", "Photons pT > 0.5(0.25) diphoton mass     : ", "Lepton veto                              : ", "At least 3 jets                          : ", "At least 1 Loose b-Jet                   : ", "ttHMVA>0.75                              : ", "Diphoton MVA > 0.4                       : ", "At least 5 jets                          : ", "At least 1 Medium b-Jet                  : ", "Diphoton MVA > 0.5                       : " };
	...
	cout << endl <<         "                                        With ttH MVA                                        " << endl << endl;

	for(int i=10; i<14; i++)
		cout << name[i] << std::setprecision(3) << events[i] << ", " << percentage[i] << "% of the total" << endl;

	cout << endl <<         "                                         Cut based                                         " << endl << endl;
	...
	system("mv *.png ~/www/ttH/Full2016Dataset/Hadronic/Efficiency");
	system("mv *.pdf ~/www/ttH/Full2016Dataset/Hadronic/Efficiency");
