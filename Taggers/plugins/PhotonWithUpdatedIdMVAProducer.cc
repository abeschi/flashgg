...
namespace flashgg {
    ...
    public:
        ...
        void storePhotonRegression(flashgg::Photon & diph, const std::string & label);
        void updatePhotonRegression(flashgg::Photon & diph);
    ...
    private:
        ...
        edm::EDGetTokenT<edm::View<flashgg::MuMuGammaCandidate> > token_;
    ...
    PhotonWithUpdatedIdMVAProducer::PhotonWithUpdatedIdMVAProducer( const edm::ParameterSet &ps ) :
        token_(consumes<edm::View<flashgg::MuMuGammaCandidate> >(ps.getParameter<edm::InputTag>("src"))),
    ...
    {
            storeRegression(ph, label);
    }

    void PhotonWithUpdatedIdMVAProducer::storeRegression(flashgg::Photon & ph, const std::string & label)
    {
    ...
    float PhotonWithUpdatedIdMVAProducer::correctPhoton( flashgg::Photon & ph ) 
    {
        if (this->debug_) {
    ...
    void PhotonWithUpdatedIdMVAProducer::correctPhoton_non5x5( flashgg::Photon & ph ) 
    {
        // if (this->debug_) {
        //     std::cout << ph.full5x5_r9() << std::endl;
        //     std::cout << ph.old_r9() << std::endl;
