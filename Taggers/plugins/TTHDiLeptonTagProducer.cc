    ...
    void TTHDiLeptonTagProducer::produce( Event &evt, const EventSetup & )
    {
        ...
        for( unsigned int diphoIndex = 0; diphoIndex < diPhotons->size(); diphoIndex++ )
        {
            ...
            if( theElectrons->size()!=0 && theMuons->size()!=0 && (theElectrons->size() + theMuons->size()) >=2)
                MixedPairs = selectEleMuon(theMuons->ptrs(), theElectrons->ptrs(), dipho, ElePtCut_, MuonPtCut_, EleEtaCuts_, MuonEtaCut_,  MuonPhotonDrCut_, ElePhotonDrCut_, EleMuDrCut_, MixedLeadingLeptonPtCut_);
